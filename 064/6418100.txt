#line 1 "test/data/lazy_segment_tree_RMQ_RUQ.test.cpp"
...
struct Mapping<Min<T>, Update<S>> {
    static constexpr void mapping(typename Min<T>::set_type& t, typename Update<S>::set_type s, int) {
        // DEBUG(t, s.value_or(-1));
        if (s.has_value()) t = s.value();
    }
};
// template <class T, class S>
// struct Mapping<Add<T>, Add<S>> {
//     static constexpr void mapping(typename Add<T>::set_type& t, typename Add<S>::set_type s, int w);
// };
// template <class T, class S>
...
#line 7 "test/data/lazy_segment_tree_RMQ_RUQ.test.cpp"

namespace bys {
void Solver::solve() {
    auto [n, q] = scanner.read<int, 2>();
    LazySegmentTree<Min<int>, Update<int>> seg(n);
